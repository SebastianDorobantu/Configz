...
group_names=['WEB', 'WEBB', 'TERM', 'VIM', 'PHOTO', 'FILES', 'CHAT', 'SOUND', 'ETC']
group_layouts = ["columns", "columns", "verticaltile", "verticaltile", "columns", "columns", "columns", "columns", "columns", "columns", "columns"]
group_labels = ["", "", "", "", "", "", "", "", ""]
...
#    else:
#        keys.append(Key([mod, "Tab"], str(int((i+1)/5)), lazy.group[nume].toscreen()))
#        keys.append(Key([mod, "Tab", "shift"], str(i+1), lazy.window.togroup(nume)))
    groups.append(
            Group(
                name=group_names[i],
